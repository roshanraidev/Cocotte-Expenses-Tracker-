...
## Simple invoice review and positioned PDF extraction (23 September 2026)

The main review now has a compact supplier/date/order-reference header, three editable product columns, a net total and SAVE INVOICE. Optional dates remain absent rather than defaulting to today or the invoice date. Administrative accounting/category fields are collapsed; no catalogue matching is required during entry. Unclassified products remain separately visible in reports; invoices without delivery dates are explicitly accessible through Undated. Effective net-per-quantity prices are labelled separately from printed prices, and unavailable dates/units do not produce invented timelines or comparable price aggregates.

The actual saved PDF exposed a content-stream ordering problem: amounts and descriptions were not emitted in visual table order. Positioned PDF text now reconstructs the table and recognizes the cases quantity separately from pack size. The saved, unedited draft was rescanned with an audit record and remains unconfirmed. Its product quantity and net line amount reconcile with the printed net total. Existing account, supplier, food-record and original-document fingerprints are unchanged.

The free server OCR also uses word positions and retries sparse segmentation. Generated JPEG/PNG invoice fixtures pass extraction and browser workflows, but an image rendered from the actual PDF still loses its boxed product table. No actual camera photo was supplied, so real-photo verification remains outstanding. Missing rows trigger explicit manual entry; the application does not claim successful product extraction. More reliable varied-layout scanning would require further OCR/layout engineering or an approved managed document-analysis integration. No paid provider or API key is configured.

Verification: 65 unit/database tests, lint, TypeScript, production webpack build, financial integration checks and complete production browser workflows pass. Both existing accounts were checked against the actual saved PDF review. Browser coverage includes supplier-free uploads, inline supplier assignment, Chef restrictions, edited net prices, save/confirmation, duplicate protection, original retrieval, cross-device persistence, reports, unchanged food allowance and responsive layouts. Original files persist in authenticated PostgreSQL storage. No reset, commit, push or deployment was performed.

## Compact Cocotte visual redesign (23 September 2026)

Applied the written forest-green, cream and muted-gold requirements after the user chose to proceed without the unavailable design-reference image. This is not an image-matched implementation. The existing sidebar/workspace selector, mobile menu, typography, cards, tables, buttons, weekly navigation and report summaries share a more compact visual system. Desktop Food dashboards have a split allowance/breakdown hero and adjacent sales, cumulative chart and recent-purchase panels; tablet and phone layouts progressively stack them. The calculation link opens the selected week's purchasing report. Cumulative chart data uses the existing exact-money aggregation, with absent actuals left blank and an accompanying numerical table.

Existing Food and Packaging features, server authentication, PostgreSQL persistence, permissions and financial logic are retained. No schema migration or business-record update is part of this redesign. Real local pages were inspected read-only at 320, 390, 768, 1024 and 1512 pixels. A week lacking stock planning continues to show the setup prompt. Tests cover the calculation-report link, cumulative chart/table, existing purchasing/confirmation workflows and responsive pages using a disposable database. No commit, push or deployment is part of this stage.

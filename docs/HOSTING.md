...
## Simplified review migration

Apply `20260923000200_simple_invoice_review` with `npm run db:migrate`, regenerate Prisma, and restart the application. This additive migration permits absent order/delivery dates, retains order numbers, adds an Unclassified category, and records whether a price was printed or calculated from reviewed net/quantity. Existing invoices and documents are preserved. Missing delivery dates are explicitly reported under Undated.

PDF extraction now uses visual text positions to reconstruct tables instead of relying on PDF content-stream order. Image OCR retries sparse-text segmentation when automatic segmentation finds no products. This is a conservative local scanner, not a guaranteed invoice recognition service: boxed tables and poor photos can still need manual rows. No paid service has been selected. A managed document-analysis service would require a provider account/API key, privacy review and user approval before integration.

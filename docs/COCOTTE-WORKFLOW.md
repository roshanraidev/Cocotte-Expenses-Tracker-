...
- **Chef or Super User:** On the invoice review page, select an active supplier and correct order/delivery dates. A Super User can create a new supplier or enable a Food supplier for Both workspaces here without leaving. A Chef can save an unknown-supplier draft awaiting a Super User. The scan never confirms spending automatically. Review descriptions, quantities, net prices, categories and units/pack sizes; compare net/VAT/gross with the original. Add/remove lines as needed. Unknown quantity/price is allowed; an unknown net amount cannot be confirmed. A net-total mismatch must be corrected before confirmation.
- **Confirm:** acknowledge amounts excluding VAT and click Confirm Invoice. A valid assigned supplier is required only at confirmation. No automatic VAT conversion occurs. Duplicate invoice numbers are blocked; possible duplicate date/total/file matches need a reason and acknowledgement. Save an unconfirmed review to resume on another device. Only confirmed non-void invoices appear in spending reports.

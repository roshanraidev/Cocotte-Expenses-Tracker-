...
2. Click **Upload & review invoice** to save the original to PostgreSQL and attempt extraction. The review page shows supplier suggestions and editable dates. Select an active packaging supplier; Super Users can create a supplier or enable a Food supplier for Both workspaces inline. Chefs can save an unknown-supplier draft for later assignment. Use **Enter invoice manually** when no file is available.
...
4. Tick the explicit **EXCLUDE VAT** confirmation and click **Confirm Invoice**. Unconfirmed reviews do not affect spending. Duplicate supplier/invoice numbers are blocked; possible duplicates without a number require an explanation. Open the same invoice on another device to verify persistence.

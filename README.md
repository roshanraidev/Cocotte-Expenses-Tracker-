...
3. Review the three-column table: **Product name, Quantity, Total price excl. VAT**. Correct or add products, and check the invoice net total. Product totals must reconcile before saving. Catalogue matching is not required; unknown categories remain Unclassified for later administration.
4. Click **SAVE INVOICE** after checking the prices exclude VAT. Only saved/confirmed invoices enter spending reports. **Save draft** preserves an unfinished review. Duplicate checks remain enforced.
5. Missing order/delivery dates and order numbers show **Not found**. Enter them under **Optional details** if known; invoice dates are never substituted. Confirmed invoices without a delivery date appear under **Undated** in Reports. Super Users can inspect retained invoice/tax/category details under **More details**.

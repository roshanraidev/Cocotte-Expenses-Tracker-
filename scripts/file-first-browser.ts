...
export async function fileFirstBrowser(admin:Page,chef:Page,prisma:PrismaClient){
 ...
 await chef.getByLabel('Category 1',{exact:true}).selectOption('PACKAGING');await chef.getByLabel('unitPrice 1',{exact:true}).fill('9');await chef.getByLabel('lineNet 1',{exact:true}).fill('18');await chef.getByLabel('Invoice net total (£)',{exact:true}).fill('18');await chef.getByLabel('VAT amount (£), if known',{exact:true}).fill('3.60');await chef.getByLabel('Gross total (£), if known',{exact:true}).fill('21.60');
 await chef.getByRole('button',{name:'Save unconfirmed review',exact:true}).click();await expect.poll(async()=> (await prisma.packagingInvoice.findUniqueOrThrow({where:{id}})).version).toBe(3);
 await chef.reload();await expect(chef.getByLabel('unitPrice 1',{exact:true})).toHaveValue('9');expect(await prisma.packagingLine.count()).toBe(0);
 await admin.goto(base+`/packaging/invoices/${id}`);await admin.getByLabel('Supplier action',{exact:true}).selectOption('create');await admin.getByLabel('New supplier name',{exact:true}).fill('Unknown Cups Ltd');await admin.getByText('I checked the original and confirm all line amounts and the invoice net total EXCLUDE VAT.',{exact:true}).click();await admin.getByRole('button',{name:'Confirm Invoice',exact:true}).click();await expect(admin.getByRole('heading',{name:'Confirmed net spending · £18.00',exact:true})).toBeVisible();
 ...
 await chef.goto(base+'/packaging/reports');await expect(chef.getByRole('row').filter({hasText:'Paper Cups'})).toContainText('£18.00');
 ...
 await admin.getByLabel('Supplier action',{exact:true}).selectOption('enable');await admin.getByLabel('Food supplier to enable',{exact:true}).selectOption({label:'Fresh Market'});await admin.getByRole('button',{name:'Save unconfirmed review',exact:true}).click();await expect.poll(async()=> (await prisma.supplier.findFirstOrThrow({where:{name:'Fresh Market'}})).packagingWorkspace).toBe(true);
 await chef.goto(base+`/packaging/invoices/${jpgId}`);await expect(chef.getByLabel('Invoice supplier',{exact:true})).toHaveValue((await prisma.supplier.findFirstOrThrow({where:{name:'Fresh Market'}})).id);await expect(chef.getByText('No reliable product rows found.',{exact:false})).toBeVisible();

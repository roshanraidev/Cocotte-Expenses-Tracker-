...
export function reportFilters(query:Record<string,string|undefined>,today=londonToday()):Filters{
 const period=['week','month','custom'].includes(query.period??'')?query.period!:'week';
...
export function summarize(lines:SpendingLine[]){
 ...
 const products=[...grouped].map(([id,group])=>{
  ...
  const first=group[0],last=group.at(-1)!;const comparable=!!first.unit&&group.every(l=>l.unit===first.unit&&l.packSize===first.packSize&&l.quantity!==null);
  const quantity=comparable?group.reduce((s,l)=>s+decimal4(l.quantity!),0n):null;const net=group.reduce((s,l)=>s+l.netPence,0n);
  const average=quantity&&quantity>0n?(net*1000000n+quantity/2n)/quantity:null;
  const comparablePrices=comparable&&group.every(l=>l.unitPrice!==null);
  const latest=comparablePrices?decimal4(last.unitPrice!):null,earliest=comparablePrices?decimal4(first.unitPrice!):null;
  return {id,name:first.productName,category:first.category,unit:first.unit,packSize:first.packSize,net,invoiceCount:new Set(group.map(l=>l.invoiceId)).size,orderDates:new Set(group.map(l=>l.orderDate)).size,quantity,average,latest,change:latest!==null&&earliest!==null?latest-earliest:null,suppliers:[...new Set(group.map(l=>l.supplierName))],history:group};
 }).sort((a,b)=>a.net===b.net?a.name.localeCompare(b.name):a.net>b.net?-1:1);
 return {total:packaging+chemical,packaging,chemical,invoiceCount:new Set(lines.map(l=>l.invoiceId)).size,products,suppliers:[...suppliers.values()].map(s=>({...s,invoiceCount:s.invoices.size})).sort((a,b)=>a.net>b.net?-1:a.net<b.net?1:0)};
}

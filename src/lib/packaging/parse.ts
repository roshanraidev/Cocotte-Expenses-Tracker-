...
export type Extraction={supplierName:string;invoiceNumber:string;invoiceDate:string;net:string;vat:string;gross:string;lines:ExtractedLine[];warnings:string[]};
...
export function parseInvoiceText(text:string):Extraction {
 const result:Extraction={supplierName:'',invoiceNumber:'',invoiceDate:'',net:'',vat:'',gross:'',lines:[],warnings:['Review every extracted value against the original invoice.']};
 ...
 for(const row of rows){
  const supplier=row.match(/^supplier\s*:\s*(.+)/i);if(supplier)result.supplierName=supplier[1];

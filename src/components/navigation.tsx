...
export function Navigation({ admin }: { admin: boolean }) {
  ...
  const links = pathname.startsWith('/packaging') ? [{href:'/packaging',label:'Dashboard',icon:LayoutDashboard},{href:'/packaging/invoices',label:'Invoices',icon:ReceiptText},{href:'/packaging/products',label:'Product catalogue',icon:Store},{href:'/packaging/reports',label:'Reports',icon:ChartNoAxesCombined},...(admin?[{href:'/packaging/suppliers',label:'Supplier Management',icon:Truck}]:[])] : [{href:'/dashboard',label:'Dashboard',icon:LayoutDashboard},{href:'/sales',label:'Daily Sales',icon:ReceiptText},{href:'/orders',label:'Supplier Purchases',icon:Truck},{href:'/reports',label:'Reports',icon:ChartNoAxesCombined},...(admin?[{href:'/admin/planning',label:'Weekly Planning',icon:SlidersHorizontal},{href:'/suppliers',label:'Supplier Management',icon:Store},{href:'/admin',label:'Administration',icon:Settings2}]:[])];

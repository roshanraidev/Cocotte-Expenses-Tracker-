...
async function scan(id:string,restaurantId:string){
 ...
 try{const result=await extractDocument(row.document.bytes,row.document.mime);const aliases=await db().packagingAlias.findMany({where:{supplierId:row.supplierId},include:{product:true}});
 ...
 data={extractedText:result.text,extraction:result.extraction as unknown as Prisma.InputJsonValue,review:review as unknown as Prisma.InputJsonValue,scanNote:result.extraction.warnings.join(' '),version:{increment:1}};
...
export async function packagingAction(_previous:FormResult,form:FormData):Promise<FormResult>{
 ...
 try{
  if(operation==='upload'){
   const upload=form.get('file');const file=upload instanceof File&&upload.size?{bytes:new Uint8Array(await upload.arrayBuffer()),filename:upload.name,mime:upload.type}:undefined;

...
async function scan(id:string,user:{id:string;restaurantId:string}){
 ...
 await db().$transaction(async tx=>{const updated=await tx.packagingInvoice.updateMany({where:{id,restaurantId,version:row.version,status:'REVIEW'},data});if(!updated.count)throw new PackagingError('The review changed while scanning. Reload; your saved edits were preserved.');await tx.auditLog.create({data:{restaurantId,actorId:user.id,action:'PACKAGING_INVOICE_SCANNED',entity:'Packaging',entityId:id,before:{review:row.review??{},version:row.version},after:{note:String(data.scanNote),version:row.version+1}}});});
}
export async function packagingAction(_previous:FormResult,form:FormData):Promise<FormResult>{
 ...
 try{
  if(operation==='upload'){
   ...
   if(file&&!row.review&&!row.extractedText)await scan(row.id,user);
  }else if(operation==='scan'){await scan(String(form.get('id')),user);}
